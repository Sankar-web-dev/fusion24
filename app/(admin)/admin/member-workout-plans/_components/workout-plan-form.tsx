...
import { WorkoutPlanFormData, WorkoutPlanUpdateFormData } from '../validations/member-workout-plan.validation';
...
export function WorkoutPlanForm({ initialData, onSubmit, isLoading }: WorkoutPlanFormProps) {
  const {
    ...
    resolver: zodResolver(initialData ? undefined : require('../validations/member-workout-plan.validation').workoutPlanSchema),
  ...
  useEffect(() => {
    if (initialData && initialData.exercises) {
      setValue('exercises', initialData.exercises);
    }
